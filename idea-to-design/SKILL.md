...
- `Post-Visual Extraction` must include image-understanding notes for each implementation-critical section, not just a brief summary: dominant geometry, visual metaphor, background/decorative layers, foreground hierarchy, exact item counts, icon source/style, spacing rhythm, density, shadows/glass effects, and what must not be simplified during coding. For GPT Image 2 outputs, explicitly translate the generated visual language into implementable CSS/component constraints before handoff.
...
- handoff boundary: after a visual source is user-approved and `idea-to-design` completes Visual Freeze, Post-Visual Extraction, and the Level 3 handoff package, routine implementation ownership transfers to `design-to-code`; `idea-to-design` should not keep redesigning during coding unless the design source is missing, stale, contradicted by product changes, or the user explicitly requests a design change
